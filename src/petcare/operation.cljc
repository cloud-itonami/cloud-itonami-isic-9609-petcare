(ns petcare.operation
  "OperationActor -- one grooming operation = one supervised actor run,
  ...
  :approved}}`. `:actuation/apply-grooming-process` and
  `:actuation/return-animal` ALWAYS reach this node when the governor
  is clean -- see `petcare.phase`.
...
(defn build
  ...
  `petcare.store/Store`).
  opts:
    :advisor      -- a `petcare.advisor/Advisor` (default: mock-advisor)
